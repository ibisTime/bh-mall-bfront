...
<script>
  ...
  import { checkRed, getLevel, outOrderDetail, payOutOrder } from 'api/baohuo';
  ...
  export default {
      ...
      checkUser(userId) {
        Promise.all([
          ...
          if (res.result == '4') {
            ...
            this.redirectPage(`购买云仓已完成，${levelInfo[0].name}授权单${formatAmount(res.amount)}元`, res.isWare === '1' ? '/woyaochuhuo' : '/chuhuoNoWare');
          } else if (res.result == '2') {
            this.confirmBtnText = '我要出货';
            this.redirectPage(`购买云仓已完成，${levelInfo[0].name}升级单${formatAmount(res.amount)}元`, res.isWare === '1' ? '/woyaochuhuo' : '/chuhuoNoWare');
    ...
    mounted() {
      this.orderCode = this.$route.query.code.split(',');
      getUser().then((res) => {
        this.level = res.level;
      });
      this.orderCode.map((item) => {
        this.loading = true;
        outOrderDetail(item).then((res) => {
          this.loading = false;
          this.list.push({
