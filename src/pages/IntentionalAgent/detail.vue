<template>
    <div class="detail">
        ...
        <div :class="['choose',index != 3 ? 'hide' : '']">
            <span class="item"  @click="downward">
        ...
        <div :class="[panelLevelShow ? 'show' : '','panel']" @click="chooseUser($event)">
            <i v-for="item in list" :id="item.code" >{{item.name}}</i>
        </div>
        <toast ref="mytoast" :text="text"></toast>
    </div>
</template>
<script>
import {allotAgent,acceptAgent,ignoreAgent,getLevel,getMySub,queryManager} from 'api/baohuo'
...
export default {
  ...
  methods:{
        ...
        accept(){
                acceptAgent(this.userId).then(res => {
                    if(res.isSuccess){
                        this.text = '接受成功'
                        this.$refs.mytoast.show()
                    }else{
  ...
  mounted(){
        ...
        queryManager().then(res => {
            console.log(res)
            this.options.manager = res.list[1].code
